...
const getNextLine = (ctx, text, forcedWidth) => {
	if (!text.length) {
		return ['', ''];
	}
	...
	while (true) {
		...
		const doesSubstringFit = forcedWidth ? ctx.measureTextWidth(textToHere).width <= forcedWidth : true;
...
const getLinesToRender = (ctx, text, forcedWidth) => {
	const textSplitOnLines = text.split(/\r\n|\r|\n/g);

	const lines = [];
	for (const textLine of textSplitOnLines) {
