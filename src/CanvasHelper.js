...
class CanvasHelper {
	...
	repaintAllLayers() {
		for (const key of ['background', 'main']) {
							...
							this.transformed(
								() => {
									this._shapesInProgress.map((shape) =>
										renderShapeToContext(this.ctx, shape, { bufferCtx: this.bufferCtx, shouldOnlyDrawLatest: true }),
									);
								},
								this.ctx,
								this.bufferCtx,
	...
	draw(shapes, ctx, retryCallback) {
		...
		const drawShapes = () => {
			shapes.map((shape) => renderShapeToContext(ctx, shape, { retryCallback }));
		};
	...
	getSnapshot(keys = null) {
		...
		const snapshot = {};
		for (const k of ['colors', 'position', 'scale']) {
			if (keys.includes(k)) {
				snapshot[k] = this[k];
			}
		}
	...
	loadSnapshot(snapshot) {
		...
		if (snapshot.colors) {
			for (const k of ['primary', 'secondary', 'background']) {
		...
		if (snapshot.shapes) {
			...
			for (const shapeRepr of snapshot.shapes) {
...
CanvasHelper.defaultOptions = defaultOptions;
