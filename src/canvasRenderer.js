...
defineCanvasRenderer(
	'SelectionBox',
	(() => {
		const _drawHandle = (ctx, { x, y }, handleSize) => {
			if (handleSize === 0) {
				return;
			}
...
const _drawRawLinePath = (ctx, points, close, lineCap) => {
	if (close == null) {
		close = false;
	}
	if (lineCap == null) {
...
const drawLinePath = (ctx, shape) => {
	_drawRawLinePath(ctx, shape.smoothedPoints);
	ctx.stroke();
};

const drawLinePathLatest = (ctx, bufferCtx, shape) => {
	if (shape.tail) {
...
const drawErasedLinePath = (ctx, shape) => {
	ctx.save();
...
const drawErasedLinePathLatest = (ctx, bufferCtx, shape) => {
	ctx.save();
	ctx.globalCompositeOperation = 'destination-out';
	bufferCtx.save();
