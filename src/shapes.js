...
const defineShape = function (name, props) {
	...
	Shape.prototype.drawLatest = function (ctx, bufferCtx, retryCallback) {
		renderShapeToContext(ctx, this, { retryCallback, bufferCtx, shouldOnlyDrawLatest: true });
	};
...
const _doAllPointsShareStyle = (points) => {
	if (!points.length) {
		return false;
	}
	const { size } = points[0];
...
const _createLinePathFromData = (shapeName, data) => {
	let x, y;
	let points = null;
	if (data.points) {
...
const linePathFuncs = {
	...
	addPoint(point) {
		...
		if (!this.smoothedPoints || this.points.length < this.sampleSize) {
			...
			this.tail = last(bspline(last(this.points, this.sampleSize), this.order), this.segmentSize * this.tailSize);
			...
			this.smoothedPoints = this.smoothedPoints
				.slice(0, this.smoothedPoints.length - this.segmentSize * (this.tailSize - 1))
				.concat(this.tail);
		}
	},
...
defineShape('SelectionBox', {
	...
	toJSON() {
		return { shape: shapeToJSON(this.shape), backgroundColor: this.backgroundColor };
	},
