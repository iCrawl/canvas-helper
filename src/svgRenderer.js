...
const renderShapeToSVG = (shape, opts) => {
	if (opts == null) {
		opts = {};
	}
...
defineSVGRenderer(
	'Image',
	(
		shape, // This will only work when embedded in a web page.
	) =>
		`\
...
defineSVGRenderer('Line', (shape) => {
	const dashString = shape.dash ? `stroke-dasharray='${shape.dash.join(', ')}'` : '';
	let capString = '';
...
defineSVGRenderer('Text', function (shape) {
	...
	const widthString = shape.forcedWidth ? `width='${shape.forcedWidth}px'` : '';
	const heightString = shape.forcedHeight ? `height='${shape.forcedHeight}px'` : '';
